...
public class AbandonPlot extends HollowPluginBaseAbstractCommandExecutor {

	private HollowCreativePlots _plugin;

	public AbandonPlot(HollowPluginBase plugin) {
		super(plugin);
	...
	public boolean execute(CommandSender sender, HollowPluginBaseDoCommandData data, String commandName, String label,
		...
		try {
			...
			if(currentPlot.cleanupCreativePlot()){
				player.sendMessage(_plugin.formatCommandResponse()+ChatColor.BLUE+"Cleaned plot in creative map!");
			} else {
				player.sendMessage(_plugin.formatCommandResponse()+ChatColor.BLUE+"Something went wrong cleaning up in the creative map!");
			}
			
			if(_plugin.deletePlotWithoutTransfer(currentPlot)) {
				player.sendMessage(_plugin.formatCommandResponse()+ChatColor.BLUE+"Deleted creative plot!");
			} else {
				player.sendMessage(_plugin.formatCommandResponse()+ChatColor.BLUE+"Something went wrong deleting this plot!");
			}
		} catch(Exception e) {}
