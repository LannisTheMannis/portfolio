...
public class DeleteWithoutMoving extends HollowPluginBaseAbstractCommandExecutor {

	private HollowCreativePlots _plugin;

	public DeleteWithoutMoving(HollowPluginBase plugin) {
		super(plugin);
	...
	public boolean execute(CommandSender sender, HollowPluginBaseDoCommandData data, String commandName, String label,
		...
		try {
			...
			_plugin.deletePlotWithoutTransfer(currentPlot);
		} catch(Exception e) {}
		...
		try {
			...
			if(!currentPlot.cleanupCreativePlot()) {
				player.sendMessage(_plugin.formatCommandResponse()+ChatColor.BLUE+"Failed to clean up plot!");

			}
			if(_plugin.deletePlotWithoutTransfer(currentPlot)) {
				player.sendMessage(_plugin.formatCommandResponse()+ChatColor.BLUE+"Plot deleted!");
			} else {
				player.sendMessage(_plugin.formatCommandResponse()+ChatColor.BLUE+"Something went wrong in deleting plot!");
