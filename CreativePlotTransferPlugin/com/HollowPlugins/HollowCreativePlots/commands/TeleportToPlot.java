...
public class TeleportToPlot extends HollowPluginBaseAbstractCommandExecutor {

	private HollowCreativePlots _plugin;

	public TeleportToPlot(HollowPluginBase plugin) {
		super(plugin);
	...
	public boolean execute(CommandSender sender, HollowPluginBaseDoCommandData data, String commandName, String label,
		...
		try {
			...
			if(parsedArgs.size()==2) {
				if(parsedArgs.get(1).toLowerCase().contains("m")) {
					currentPlot.teleportToMain(player);
				} else {
					currentPlot.teleportToCreative(player);
